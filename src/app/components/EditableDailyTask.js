...
export default function EditableDailyTask(props) {
    ...
    return (
        <div    className={` select-none text-lg font-semibold shadow-md bg-secondary border-2 rounded-lg w-15vw h-12 flex  items-center mt-2  transition duration-300 ease-in-out border-gray-500 border-dashed`}>
                {editing?<input placeholder="labas" onChange={handleChange} onBlur={() =>{props.handleEdit(props.id, text); setEditing(false)}} />:
                <div className="flex flex-row justify-between">
                    ...
                    <div className="flex flex-row m-2">
                        ...
                        <button onClick={() => props.handleDelete(props.id)} className="w-6 h-6 "><IoIosRemoveCircleOutline  color="red" className="  w-6 h-6" /></button>
                    </div>
                </div>}
            </div>
    )
